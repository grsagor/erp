<form id="partnerEditForm" method="post" enctype="multipart/form-data">
    ...
    <div class="modal-body">
        ...
        <div class="col-sm-12 tab-content" id="v-pills-tabContent">
            <div class="step step_1 tab-pane fade active show">
                <div class="form-group mb-2">
                    <select name="type_of_event" class="form-control type_of_event" id="type_of_event" required>
                        ...
                        <option {{ $event->type_of_event == 'golder_guiter' ? 'selected' : '' }} value="golder_guiter">Golden Guiter</option>
                        <option {{ $event->type_of_event == 'regular' ? 'selected' : '' }} value="regular">Regular</option>
                    </select>
                </div>

                <div class="gap-1 mb-1 w-100 title-container {{ $event->type_of_event == 'golder_guiter' ? 'd-flex' : 'd-none' }}">
                ...
                @foreach ($event_artists as $index => $artist)
                    <div class="form-group mb-1">
                        <select name="artist_id[]" class="form-control" id="artist_id">
                            <option value="">Select Artist</option>
                            @foreach($artists as $row)
                                <option {{ $artist->artist_id == $row->id ? 'selected' : '' }} value="{{ $row->id}}">{{ $row->first_name}} {{ $row->last_name }}</option>
                            @endforeach
                        </select>
                    </div>
                @endforeach
                ...
                <div class="form-group mb-2">
                    <select name="venue_id" class="form-control mb-1" id="sponsor_id">
                        ...
                        @foreach($venues as $row)
                            <option {{ $event->venue_id == $row->id ? 'selected' : '' }} value="{{ $row->id}}">{{ $row->name}}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="d-flex gap-2 mb-1 w-100">
                    <div class="form-floating mb-1 w-100">
                        <input type="number" class="form-control" placeholder="Email"
                            name="max_participent" value="{{ $event->max_participent }}">
                        <label for="">Max Participant</label>
                    </div>
                    <div class="form-floating mb-1 w-100">
                        <input type="number" class="form-control" placeholder="Amount"
                            value="{{ $event->amount }}" name="amount" required>
                ...
                <div class="d-flex w-100 gap-2 mb-1">
                    <div class="form-group mb-1 w-100">
                        <input type="file" class="form-control"
                            ...
                            name="images[]" id="profile_image" multiple>
                        <img src="{{ $event->banner && $event->banner[0] ? asset('uploads/event-images/'.$event->banner[0]) : asset('assets/utils/images/no-img.jpg') }}" height="80px" width="100px"
            ...
            <div class="step step_3 tab-pane fade">
                ...
                <div class="form-floating mb-1 w-100">
                    <input type="text" class="form-control" placeholder="Email"
                        name="booking_url"  value="{{ $event->booking_url }}" required>
                    <label for="">Booking url<span class="text-danger">*</span></label>
                </div>
                <div class="d-flex gap-1 mb-1 w-100">
                    <div class="form-floating mb-1 w-100">
                        <textarea type="text" class="form-control h-auto" name="description" placeholder="Description" rows="5" required>{{ $event->description }}</textarea>
                ...
                <div class="d-flex gap-1 mb-1 w-100">
                    <div class="form-floating mb-1 w-100">
                        <input type="date" class="form-control" placeholder="Title"
                            value="{{ $event->publish_date }}" name="publish_date">
                ...
                <div class="d-flex gap-2 mb-1 w-100">
                    <div class="form-floating mb-1 w-100">
                        <input type="number" class="form-control" placeholder="Email"
                            name="sponsour_min_value" value="{{ $event->sponsour_min_value }}">
                        <label for="">Sponsor minimum value</label>
                    </div>
                    <div class="form-floating mb-1 w-100">
                        <input type="number" class="form-control" placeholder="Amount"
                            value="{{ $event->sponsour_max_value }}" name="sponsour_max_value">
                ...
                <div class="d-flex gap-2 mb-1 w-100">
                    <div class="form-group mb-1 w-100">
                        <select name="discount_type" class="form-control" id="discount_type">
                            ...
                            <option {{ $event->discount_type == 'percentage' ? 'selected' : '' }} value="percentage">Percentage</option>
                            <option {{ $event->discount_type == 'fixed' ? 'selected' : '' }} value="fixed">Fixed</option>
                        </select>
                    </div>
                    <div class="form-floating mb-1 w-100">
                        <input type="number" class="form-control" placeholder="Amount"
                            value="{{ $event->discount_value }}" name="discount_value">
                ...
                <div class="row">
                    ...
                    <div class="col-sm-3 d-flex align-items-center">
                        <div class="form-check form-switch">
                            <input {{ $event->status == 1 ? 'checked' : '' }} class="form-check-input" type="checkbox" name="status"
                    ...
                    <div class="col-sm-3 d-flex align-items-center">
                        <div class="form-check form-switch">
                            <input {{ $event->is_free == 1 ? 'checked' : '' }} class="form-check-input" type="checkbox" name="is_free"
                                id="flexSwitchCheckDefault">
                        </div>
                    </div>
                </div>
            ...
            <div class="step step_4 tab-pane fade edit">
                <div class="w-100 text-left">
                    <label for="">Sponsor Details</label>
                    <hr>
                </div>
                <table class="w-100 edit-sponsor">
                    <thead>
                        <tr>
                            ...
                            <th class="w-5 text-center">
                                <button href="#" type="button"
                                    onclick="incrementRow('edit-sponsor', 'itwillbecoppy'); return false;"
                    ...
                    <tbody>
                        @foreach ($event_sponsors as $index => $sponsor)
                        <tr class="itwillbecoppy" data-row-no="{{ $index }}">
                            <td>
                                <div class="form-group mb-1">
                                    <select name="sponsor_id[]" class="form-control" id="sponsor_id" required>
                                        <option value="">Select Sponsor</option>
                                        @foreach($sponsors as $row)
                                            <option {{ $sponsor->sponsor_id == $row->id ? 'selected' : '' }} value="{{ $row->id}}">{{ $row->first_name}} {{ $row->last_name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </td>
                            <td>
                                <div class="form-floating mb-1 w-100">
                                    <input type="text" class="form-control" placeholder="Amount"
                                        value="{{ $sponsor->amount }}" name="sponsor_amount[]">
            ...
            <div class="step step_5 tab-pane fade create">
                ...
                <div class="d-flex gap-1 mb-1 w-100">
                    <div class="form-floating mb-1 w-100">
                        <input type="text" class="form-control" name="facebook_link" placeholder="Facebook Link" @if ($social_media_links)value="{{ $social_media_links->facebook_link }}"@endif>
                        <label for="">Facebook Link</label>
                    </div>
                </div>
                <div class="d-flex gap-1 mb-1 w-100">
                    <div class="form-floating mb-1 w-100">
                        <input type="text" class="form-control" name="spotify_link" placeholder="Spotify Link" @if ($social_media_links)value="{{ $social_media_links->spotify_link }}"@endif>
                        <label for="">Spotify Link</label>
                    </div>
                </div>
                <div class="d-flex gap-1 mb-1 w-100">
                    <div class="form-floating mb-1 w-100">
                        <input type="text" class="form-control" name="itunes_link" placeholder="iTunes Link" @if ($social_media_links)value="{{ $social_media_links->itunes_link }}"@endif>
                        <label for="">iTunes Link</label>
                    </div>
                </div>
                <div class="d-flex gap-1 mb-1 w-100">
                    <div class="form-floating mb-1 w-100">
                        <input type="text" class="form-control" name="youtube_link" placeholder="Youtube Link" @if ($social_media_links)value="{{ $social_media_links->youtube_link }}"@endif>
                        <label for="">Youtube Link</label>
                    </div>
                </div>
                <div class="d-flex gap-1 mb-1 w-100">
                    <div class="form-floating mb-1 w-100">
                        <input type="text" class="form-control" name="instagram_link" placeholder="Instagram Link" @if ($social_media_links)value="{{ $social_media_links->instagram_link }}"@endif>
